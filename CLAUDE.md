...
- **Deploy target: Render Static Site, from the repo root's `render.yaml` (added 2026-09-21j)** — this project's own folder is *not* the git repo root (the repo root is one level up, `AI_Learning_Operations_ERP/`, holding the spec docs and other project folders alongside this one). A Render Static Site has to be told this folder specifically is what to publish (`rootDir` + `staticPublishPath: .` in `render.yaml`, or the same two values set by hand in the dashboard on a manually-created service) — otherwise Render can't find `index.html` at what it thinks is the site root, and every relative `css/`/`js/`/`src/` request 404s while the raw (unstyled, unscripted) HTML may still partially render. This exact failure mode shipped once — see the README's "Deploying to Render" section for the symptom and the fix. No app code should ever assume a specific deploy path beyond "this folder is the site root" (already true everywhere per §18's own relative-path rules); this is a deploy-config concern, not a code one.
